...
func reportHeader(pdf *gopdf.GoPdf, mainTitle, title, qrs, doi string) {
	...
	x, y := addMultiLineBlock(pdf, xp, yp, titleWidth, 30, "UNITED REPUBLIC OF TANZANIA", true)

	pdf.SetTextColor(0, 122, 204)
	x, y = addMultiLineBlock(pdf, x, y, titleWidth, 30, strings.ToUpper(mainTitle), true)

	pdf.SetTextColor(0, 0, 0)
	x, y = addMultiLineBlock(pdf, x+10, y, titleWidth, 15.0, "NTP", true)
